...
public class MenuDao{
    ...
    @Transactional
    public int edit(final PageFormData formData,final List<String> kids){
        final int rows = dao.execute("sys_menu.edit",formData);
        final Object relation = formData.get("relation");
        final ArrayList<HashMap<String,String>> lists = new ArrayList<>();
        kids.forEach((kid)->{
            final HashMap<String,String> map = new HashMap<>();
            map.put("kid",kid);
            map.put("relation",relation+"@"+kid);
            lists.add(map);
        });
        if(lists.size() > 0){
            dao.execute("sys_menu.updateRelations",lists);
        }
        return rows;
    }
